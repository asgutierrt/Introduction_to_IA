...
def subplots(X,G0_list,plot_dims,annotations_list,norma,html_name=''):
  ...
  for fig_ix in range(n_subplots):
    sub_fig = plot_clusters(X,G0_list[fig_ix],plot_dims,annotations_list[fig_ix],norma)
  ...
  titulo='naive classes with norm %s: crear todas las cajas desde un unico punto de referencia'%norma
